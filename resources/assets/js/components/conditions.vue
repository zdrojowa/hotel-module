    ...
    <div>
        ...
        <div v-for="lang in langs" class="row">
            <div class="form-group col-sm-12">
                ...
                <ckeditor :editor="editor" v-model="conditions[lang.key]" :config="editorConfig"></ckeditor>
            </div>
        </div>
    </div>
</template>
        ...
        methods: {
                    ...
                    axios.get('/api/offers?id=' + self._id)
                        .then(res => {
                            if (res.data.conditions != null) {
                                self.conditions = res.data.conditions;
                                self.checkLangs();
                            }
                            self.checkLangs();
                        }).catch(err => {
            ...
            checkLangs: function() {
                ...
                self.langs.forEach(lang => {
                    if (!(lang.key in self.conditions)) {
                        self.conditions[lang.key] = '';
                    }
                });
            },
            ...
            save: function(e) {
                ...
                axios.post('/dashboard/offers/' + this._id, formData, {
                    ...
                    .then(res => {
                        this.$bvToast.toast('Warunki zaktualizowane', {
                            title: `Warunki`,
                            variant: 'success',
                            solid: true
                        })
